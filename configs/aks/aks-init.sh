...
az aks create --node-count 2 --tier free --disable-defender --resource-group $resourceGroup --location $location --name $clusterName --network-plugin "azure" --network-plugin-mode "overlay" --network-dataplane "azure" --pod-cidr $AKS_POD_CIDR --generate-ssh-keys --kubernetes-version $AKS_K8S_VERSION --node-vm-size "$AKS_SIZE_POOL_DEFAULT" --vnet-subnet-id "/subscriptions/$AZ_SUBSCRIPTION/resourceGroups/rg-dev-pick/providers/Microsoft.Network/virtualNetworks/vnet-aks-pick/subnets/aks-pool-default"